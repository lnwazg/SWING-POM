...
import com.lnwazg.server.RMIServer;
...
public class MainFrame extends XmlJFrame
{
    ...
    private void initListeners()
    {
        switchBtn.setOnCallback(() -> {
            ...
            // 开启代理MQ
            RMIServer.getInstance(localPortInteger).start();
        });
        switchBtn.setOffCallback(() -> {
            // 关闭代理MQ
            WinMgr.setConfig("switch", "false");
            RMIServer.getInstance(localPortInteger).shutdown();
            Logs.i("服务器已经关闭...");
            ExecMgr.guiExec.execute(() -> {
