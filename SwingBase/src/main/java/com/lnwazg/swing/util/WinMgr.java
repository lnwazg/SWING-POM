...
public class WinMgr
{
     ...
     * 默认的sqlite的数据库名称
     */
    private static String DEFAULT_SQLITE_DB_NAME = "sqliteDb.db";
    
    /**
     * 获取默认的sqlite的MyJdbc连接信息对象<br>
     * 特供给Swing应用的
     * @author nan.li
     * @return
     */
    public static MyJdbc getDefaultSqliteMyJdbc()
    {
        //sqliteDbName=JobControlSystem
        String sqliteDbName = WinMgr.cfg("sqliteDbName");
        if (StringUtils.isEmpty(sqliteDbName))
        {
            Logs.w("配置文件config.properties中未能找到配置项：sqliteDbName，因此即将采用默认的sqliteDbName：" + DEFAULT_SQLITE_DB_NAME);
            sqliteDbName = DEFAULT_SQLITE_DB_NAME;
        }
        MyJdbc jdbc = DbKit.getJdbc("jdbc:sqlite://" + LocalUiLoader.CONFIG_FILE_DIR + File.separator + sqliteDbName, "", "");
        return jdbc;
    }
    
    /**
     * 获取默认的mysql的MyJdbc连接信息对象<br>
     * 特供给Swing应用的
     * @author nan.li
     * @return
     */
    public static MyJdbc getDefaultMysqlMyJdbc()
    {
        return DbKit.getDefaultJdbc();
    }
